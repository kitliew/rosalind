...
def read_fasta(filename):
    with open(filename) as file:
        dna = file.read().partition("\n")[-1].replace("\n","")
    return dna

def possibility_pm(filename):
    dna = read_fasta(filename)
    _a_count = dna.count("A")
    _g_count = dna.count("G")
    possible_a = 1
    possible_g = 1
    for i in range(1,_a_count+1):
        possible_a *= i
    for j in range(1,_g_count+1):
        possible_g *= j
    return possible_g * possible_a


print(possibility_pm(sys.argv[1]))