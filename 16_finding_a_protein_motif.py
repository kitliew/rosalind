...
def find_protein_motif(ID):
    ...
    all_target=re.findall("N[^P][ST][^P]", protein, overlapped=True)
    #or all_target=re.findall(r'(?=N[^P][ST][^P])', protein)
...
def find_iloc_protein_motif(ID):
    ...
    loc_list=[]
    id, protein = url_content(ID)
    for m in re.finditer("N[^P][ST][^P]", protein, overlapped=True):
    #or for m in re.finditer(r'(?=N[^P][ST][^P])', protein):        #r"PRE(?=POST)  Matches if POST matches after PRE. Doesn't consume any string
